...
$categoryQuery = $database->query("SELECT  * FROM incomes_category_assigned_to_users WHERE user_id={$_SESSION['loggedId']}");
...
if (isset($_POST['amount']))
{
	...
	if ($isAllOk)
	{
		$query = $database -> query ("INSERT INTO incomes (id, user_id, income_category_assigned_to_user_id, amount, date, comment) VALUES (NULL, {$_SESSION['loggedId']}, {$_POST['category']}, {$amount}, '{$_POST['date']}', '{$_POST['comment']}')");
		...
		$_SESSION['upgrade'] = "Dodano nowy przychód!";
